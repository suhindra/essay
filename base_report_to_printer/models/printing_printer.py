...
class PrintingPrinter(models.Model):
    ...
    @api.multi
    def print_document(self, report, record_ids, content, format, copies=1):
        """ Print a file
        ...
        return self.print_file(
            file_name, report=report, record_ids=record_ids, copies=copies, format=format)

    @api.multi
    def print_file(self, file_name, report=None, record_ids=0, copies=1, format=None):
        """ Print a file """
        self.ensure_one()
        ...
        if report == 'Barcode':
            ...
            job_count=self.env['printing.job'].search_count([('name','=',str(report.name) + str(record_ids))])
            if (job_count < report.max_print) or (report.print_limit_exception.id == self.env.uid):
                connection.printFile(self.system_name,
                                     file_name,
                                     str(report.name) + str(record_ids),
                                     options=options)
                _logger.info("Printing job: '%s' on %s" % (
                    file_name,
                    self.server_id.address,
