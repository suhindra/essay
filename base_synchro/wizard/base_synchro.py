...
class base_synchro(models.TransientModel):
    ...
    def synchronize(self, server, object):
        pool = registry(self.env.cr.dbname)
        self.meta = {}
        ids = []
        ...
        for dt, id, action in ids:
            ...
            if id2:
                ...
                self.env['base.synchro.obj.line'].create({'obj_id': object.id, 'local_id': (action == 'u') and id or idnew, 'remote_id': (action == 'd') and id or idnew})
                self.report_total += 1
...
#        return {}
        if syn_obj.user_id:
            ...
            request = registry(cr.dbname).get('res.request')
            if not self.report:
